...
func runTable() []runCase {
	return []runCase{
		...
		},
		{
			...
			guards: model.Guards{
				Now: now, LeaseValid: true, CancelRequested: true,
				AllStepsTerminal: true, AnyStepCancelled: true, ReasonCode: reasonCode,
			},
			want: model.RunCancelled,
		...
		{
			name:  "a cancellation that arrives after a failure records the failure",
			from:  model.RunRunning,
			event: model.EvCancelObserved,
			guards: model.Guards{
				Now: now, LeaseValid: true, CancelRequested: true,
				AllStepsTerminal: true, AnyStepFailed: true, AnyStepCancelled: true,
		...
		{
			name:  "a cancellation that arrives after the last step succeeded records the success",
			from:  model.RunRunning,
			event: model.EvCancelObserved,
			guards: model.Guards{
				Now: now, LeaseValid: true, CancelRequested: true,
				AllStepsTerminal: true, ReasonCode: reasonCode,
			},
			want: model.RunSucceeded,
			effects: with(kinds(model.EffectKillProcessGroup, model.EffectSetFinished, model.EffectReleaseLease),
				emit("run.succeeded")),
		...
		{
			name:  "a cancellation cannot be observed while a step is still open",
			from:  model.RunRunning,
			event: model.EvCancelObserved,
			guards: model.Guards{
				Now: now, LeaseValid: true, CancelRequested: true,
		...
		},
		{
			...
			event: model.EvCancelObserved,
			guards: model.Guards{
				Now: now, CancelRequested: true, AllStepsTerminal: true,
				AnyStepCancelled: true, ReasonCode: reasonCode,
			},
			want: model.RunRunning,
			err:  model.ErrStaleLease,
		},
		{
			...
			guards: model.Guards{
				Now: now, LeaseValid: true, CancelRequested: true,
				AllStepsTerminal: true, AnyStepCancelled: true,
			},
			want: model.RunRunning,
			err:  model.ErrMissingReasonCode,
		},
...
func stepTable() []stepCase {
	return []stepCase{
		...
		{
			// A never-started step is cancelled, not skipped: the
			// aggregate (I10) has to read the run's verdict off its steps.
			name:    "a cancellation of a pending step cancels it",
			from:    model.StepPending,
			event:   model.EvCancelObserved,
			guards:  model.Guards{Now: now, CancelRequested: true, ReasonCode: reasonCode},
			want:    model.StepCancelled,
			effects: with(kinds(model.EffectSetFinished), emit("step.cancelled")),
		},
		{
			name:   "cancelling a pending step needs a reason code too",
			from:   model.StepPending,
			event:  model.EvCancelObserved,
