...
func TestPrunePlansEstimatesWithoutTouchingAnything(t *testing.T) {
	...
	// Two batches for runs - the delete, then the probe that comes back
	// empty - plus one empty probe per remaining rule.
	if res.Batches != 6 {
		t.Fatalf("pass made %d batches, want 2 for runs plus one per idle rule", res.Batches)
	}
}
