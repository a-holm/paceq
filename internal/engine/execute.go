...
func (d *drive) execute(ctx context.Context, e *Engine) (string, error) {
	...
	verdict, detail, err := e.finishReason(ctx, d.runID, timedOut, d.job.Timeout)
	if err != nil {
		return "", err
	}
...
// was cancelled; otherwise the run succeeded, a skip counting as success. The
// detail comes back too: notification planning (#29) reads the same rows the
// verdict did, so the alert can never disagree with the state change it is
// written in.
//
...
) (runVerdict, store.RunDetail, error) {
	detail, err := e.Store.GetRun(ctx, runID)
	if err != nil {
