...
`make ci` walks its targets through `scripts/gate-run.sh`, which skips a target only when this exact tree has already passed it. The proof is a content key over the working tree: the path, executable bit and bytes of every tracked and untracked-but-not-ignored file, the Go toolchain, the `go env` settings that change what it produces, the make variables the environment can override, and whether the optional tools the gate branches on are installed. Only the runner writes a stamp, and only after `make` exited zero, so no hand can stamp a tree the gate never saw. Stamps live in the per-worktree git directory, cannot be committed, and expire after twelve hours (`PACEQ_GATE_STAMP_TTL`, in seconds) because the vulnerability database `govulncheck` reads is an input no key can see. Anything missing, stale, damaged or unreadable runs the target. Every push reports how many targets it skipped, and `PACEQ_GATE_STAMP=0` runs everything regardless. The stamp does not cover files git ignores, so a target that reads one can go stale behind it.
